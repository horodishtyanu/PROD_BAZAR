...
$(document).on('click', '.header-menu-open', function (event) {
  ...
  $('.header-menu-open, .header-menu-close').addClass('active');
  return false;
});
$(document).on('click', '.header-menu-close', function (event) {
  closeHeaderMenu();
  return false;
});
$(document).on('click', function (event) {
  if ($(event.target).closest('.header-menu-open, .header-menu-close, .header-menu-body').length) return;
  closeHeaderMenu();
});
$(document).on('click', '.header-menu-is-open .header-menu .menu-item-has-children>a>i', function (event) {
  ...
  if (li.hasClass('active')) {
    ...
    closeMenu();
    li.addClass('active');
    li.children('.sub-menu').slideDown(400);
    li.children('.children').slideDown(400);
  }

  function closeMenu() {
    li.parent().children('.menu-item-has-children').removeClass('active');
    li.parent().children('.menu-item-has-children').children('.sub-menu').slideUp(400);
  }
...
function closeHeaderMenu() {
  if (!$('html').hasClass('header-menu-opened')) return;
  $('html').removeClass('header-menu-is-open');
  $('.header-menu-open, .header-menu-close').removeClass('active');
  setTimeout(function () {
...
$(document).on('click', '.header-search-open', function () {
  closeHeaderMenu();
  $('.header-search .form-block').removeClass('alert');
  $('html').addClass('header-search-is-open');
  setTimeout(function () {
...
$(document).on('click', function (event) {
  if ($(event.target).closest('.header-search, .header-search-open, .header-search-close').length) return;
  closeHeaderSearch();
});

function closeHeaderSearch() {
  $('html').removeClass('header-search-is-open');
}
/* form */
...
$(document).on('input change', '[required]', function () {
  // var t = $(this);
  // t.closest('.form-block').addClass('valid focus');
  var t = $(this);
  setTimeout(function () {
    validateInput(t);
  }, 100);
...
$(document).on('click', '.password-input-toggle', function () {
  var input = $(this).closest('.form-block').find('.input');
...
$(document).on('submit', '.form', function () {
  ...
  t.find('[required]').each(function () {
    var validInput = validateInput($(this));
    if (!validInput) validForm = false;
  });
  ...
  t.find('[name][type="checkbox"], [name][type="radio"]').each(function (index) {
    ...
    if (t.prop('checked')) {
      formData.append(t.attr('name'), t.val());
    }
  });
  var file = t.find('.file');
  var files;

  if (file[0]) {
    files = t.find('.file')[0].files;
  }
  ...
  $.ajax({
    ...
    success: function success(answer) {
      ...
      t.find('.input').val('').closest('label').removeClass('valid alert focus');
      t.find('.file').val('');
      t.find('.file-label-title').text(t.find('.file-label-title').data('title')).removeClass('active');
      t.find('[name][type="checkbox"], [name][type="radio"]').prop('checked', false).closest('label').removeClass('active');
      var sentText = t.data('sent-text');

      if (sentText) {
        $('#modal-sent .modal-title-desc').html(sentText);
      } else {
        $('#modal-sent .modal-title-desc').html($('#modal-sent .modal-title-desc').data('sent-text'));
      }
...
var prevSliderArrow = '<button type="button" class="slick-prev"><svg><use xlink:href="#icon-angle-left"></use></svg></button>';
var nextSliderArrow = '<button type="button" class="slick-next"><svg><use xlink:href="#icon-angle-right"></use></svg></button>';
...
$('.partners-slider').slick({
  ...
  autoplay: true,
  autoplaySpeed: 6000,
  appendArrows: $('.partners-slider-body'),
  appendDots: $('.partners-slider-wrapp'),
  slidesToShow: 6,
  responsive: [{
    breakpoint: 1023,
    settings: {
...
$('.other-products-slider').slick({
  accessibility: false,
  speed: 600,
  appendArrows: $('.other-products-title'),
  prevArrow: prevSliderArrow,
  nextArrow: nextSliderArrow,
  slidesToShow: 4,
  responsive: [{
...
$(document).on('click', '.select-list a', function () {
  var t = $(this);
  var val = t.data('value') || t.text();
  var wrapp = t.closest('.select');
  t.closest('.select-list').find('li').removeClass('active');
  t.closest('li').addClass('active');
  wrapp.removeClass('active');
  wrapp.find('.select-input').val(val).trigger('blur');
  wrapp.find('.select-title').text(val);
  return false;
});
$(document).on('click', function (event) {
...
$(document).on('click', '.catalog-filter-list-toggle', function () {
  var t = $(this);
  ...
  t.closest('.catalog-filter-block').find('.catalog-filter-list-more').slideToggle(400);
  return false;
});
$('.range-slider').each(function () {
  ...
  slider.slider({
    ...
    create: function create() {
      inputFrom.val(formatPrice(valueFrom));
      inputTo.val(formatPrice(valueTo));
    },
    slide: function slide(event, ui) {
      inputFrom.val(formatPrice(ui.values[0]));
      inputTo.val(formatPrice(ui.values[1]));
  ...
  inputFrom.on('blur', function () {
    var t = $(this);
    var val = getPriceValue(t.val());
    var valueTo = getPriceValue(inputTo.val());

    if (!val) {
      t.val(formatPrice(value));
      return;
    }

    if (val < min || val > valueTo) {
      t.val(formatPrice(min));
    } else {
      t.val(formatPrice(t.val()));
    }

    slider.slider('values', [getPriceValue(t.val()), valueTo]);
    checkValues(getPriceValue(t.val()), valueTo);
  });
  inputTo.on('blur', function () {
    var t = $(this);
    var val = getPriceValue(t.val());
    var valueFrom = getPriceValue(inputFrom.val());
    ...
    if (val > max || val < valueFrom) {
      t.val(formatPrice(max));
    } else {
      t.val(formatPrice(t.val()));
    }

    slider.slider('values', [valueFrom, getPriceValue(t.val())]);
    checkValues(valueFrom, getPriceValue(t.val()));
  });
  $('.range-input').on({
  ...
  function checkValues(vFrom, vTo) {
    if (vFrom != valueFrom) {
      $('.ui-slider-handle:first').addClass('active');
    } else {
      $('.ui-slider-handle:first').removeClass('active');
    }

    if (vTo != valueTo) {
      $('.ui-slider-handle:last').addClass('active');
    } else {
      $('.ui-slider-handle:last').removeClass('active');
    }
  }
});
...
$(document).on('click', '.catalog-sort-open', function (event) {
  $('html').addClass('catalog-sort-opened catalog-sort-is-open');
  return false;
});
$(document).on('click', '.catalog-sort-close', function (event) {
  closeCatalogSort();
  return false;
});
$(document).on('click', function (event) {
  if ($(event.target).closest('.catalog-sort-open, .catalog-sort-body').length) return;
  closeCatalogSort();
});

function closeCatalogSort() {
  if (!$('html').hasClass('catalog-sort-opened')) return;
  $('html').removeClass('catalog-sort-is-open');
  setTimeout(function () {
    $('html').removeClass('catalog-sort-opened');
  }, 400);
}
/* catalog-filter */
...
$(document).on('click', function (event) {
  if ($(event.target).closest('.catalog-filter-open, .catalog-filter-body').length) return;
  closeCatalogFilter();
});

function closeCatalogFilter() {
  if (!$('html').hasClass('catalog-filter-opened')) return;
...
(function () {
  $(document).on('click', '.select-number-btn', function (event) {
    event.preventDefault();
    var t = $(this);
    var input = t.closest('.select-number').find('.select-number-input');
    var val = input.val();
    var min = input.data('min') || 0;
    var max = input.data('max') || Infinity;
    var step = input.data('step') || 1;
    var desc = input.data('desc') || ' шт.';
  ...
  $(document).on('blur', '.select-number-input', function () {
    ...
    if (val < min) {
      t.val(min + desc);
      pricingCart(t, min, min);
      return;
    }

    if (val > max) {
      t.val(max + desc);
      pricingCart(t, max, min);
      return;
    }
  ...
  function pricingCart(context, value, min) {
    var product = context.closest('.cart-product');
    var price = getPriceValue(product.find('.cart-product-price-cost-total').text());

    if (value == min) {
      product.find('.cart-product-price-sum .cart-product-price-desc').removeClass('active');
    } else {
      product.find('.cart-product-price-sum .cart-product-price-desc').addClass('active');
    }

    product.find('.cart-product-price-sum-count').text(value);
    var sum = value * price;
    product.find('.cart-product-price-sum-total').text(formatPrice(sum));
  }
})();